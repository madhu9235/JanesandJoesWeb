				...
				actions : {
							...
							success : function (aEntityData) {
								this.waitFor({
									...
									success : function (oList) {
										var sCurrentId,
											aItemsNotInTheList = aEntityData.filter(function (oObject) {
												return !oList.getItems().some(function (oListItem) {
													return oListItem.getBindingContext().getProperty("OPN") === oObject.OPN;
												});
											});

										if (!aItemsNotInTheList.length) {
											// Not enough items all of them are displayed so we take the last one
											sCurrentId = aEntityData[aEntityData.length - 1].OPN;
										} else {
											sCurrentId = aItemsNotInTheList[0].OPN;
										}
										...
										oCurrentItem.bindingPath = "/" + oList.getModel().createKey("ZtvsCollection", {
											OPN : sCurrentId
										});
					...
					iRememberTheListItem : function (oListItem) {
						...
						this.getContext().currentItem = {
							...
							id: oBindingContext.getProperty("OPN"),
							title: oBindingContext.getProperty("OPNDESC")
						};
					}
				},
				...
				assertions : {
					...
					theListHeaderDisplaysZeroHits : function () {
						return this.waitFor({
							viewName : sViewName,
							id : "page",
							matchers : new PropertyStrictEquals({name : "title", value : "ZtvsCollection (0)"}),
							success : function () {
								Opa5.assert.ok(true, "The list header displays 'ZtvsCollection (0)'");
							},
					...
					theListShouldBeSortedAscendingOnName : function () {
						return this.theListShouldBeSortedAscendingOnField("OPNDESC");
					},
					...
					theListShouldHaveAllEntries : function () {
						...
						// retrieve all ZtvsCollection to be able to check for the total amount
						...
						return this.waitFor({
							...
							success : function (oList) {
								...
								this.waitFor({
									id : "page",
									viewName : sViewName,
									matchers : new PropertyStrictEquals({name : "title", value : "ZtvsCollection (" + iExpectedLength + ")"}),
									success : function () {
